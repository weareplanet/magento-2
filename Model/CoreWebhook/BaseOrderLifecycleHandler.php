...
abstract class BaseOrderLifecycleHandler extends DefaultWebhookLifecycleHandler
{
     ...
     * non-WeArePlanet payment method: the Portal will eventually
    ...
    public function preProcess(WebhookListener $listener, WebhookContext $context): bool
    {
        ...
        if (!$this->sdkEntity) {
            $this->logger->debug(sprintf(
                'No SDK entity for %s %d — likely a Portal transaction abandoned because the order was placed with a non-WeArePlanet payment method. Acking webhook.',
        ...
        if (!$this->order) {
            $this->logger->debug(sprintf(
                'No Magento order linked to %s %d — likely a Portal transaction abandoned because the order was placed with a non-WeArePlanet payment method. Acking webhook.',
                $listener->getTechnicalName(),
                $context->entityId
            ));
            return false;
     ...
     * after a non-WeArePlanet checkout), we return only the base
    ...
    public function getLockableResources(WebhookListener $listener, WebhookContext $context): array
    {
        ...
        if ($orderId) {
            $locks[] = 'weareplanet_order_update_' . $orderId;
        }
        return $locks;
    }
