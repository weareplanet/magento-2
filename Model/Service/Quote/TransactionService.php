 ...
 */
namespace WeArePlanet\Payment\Model\Service\Quote;
...
use WeArePlanet\Sdk\Model\CustomersPresence;
...
class TransactionService extends AbstractTransactionService
{
        ...
        if (isset($transactionArray[$quote->getId()]) && $transactionArray[$quote->getId()] !== null) {
            ...
            if ($transactionInSession->getState() == TransactionState::PENDING) {
                $transaction = $this->apiClient->getService(TransactionApiService::class)->read(
                    $quote->getWeareplanetSpaceId(),
                    $quote->getWeareplanetTransactionId()
                );
            }
    ...
    private function updateTransactionByQuote(Quote $quote)
    {
        for ($i = 0; $i < self::NUMBER_OF_ATTEMPTS; $i ++) {
            try {
                ...
                if ($quote->getWeareplanetSpaceId() != $spaceId) {
        ...
        if ($transaction instanceof TransactionCreate) {
            $transaction->setSpaceViewId(
                $this->scopeConfig->getValue(
                    'weareplanet_payment/general/space_view_id',
                    ScopeInterface::SCOPE_STORE,
                    $quote->getStoreId()
                )
            );
    ...
    protected function getCustomerEmailAddress($customerEmailAddress, $customerId)
    {
        ...
        return $this->checkoutSession->getWeArePlanetCheckoutEmailAddress();
    }
    ...
    private function isGdprEnabled($storeId = null)
    {
        return $this->scopeConfig->getValue(
            'weareplanet_payment/gdpr/gdpr_enabled',
            ScopeInterface::SCOPE_STORE,
            $storeId
        ) === 'enabled';
    }
    ...
    private function convertQuoteBillingAddress(Quote $quote)
    {
        ...
        if ($gdprEnabled) {
            ...
            // paying with a non-WeArePlanet method, the pending transaction left
            // on the Portal contains no PII. The WeArePlanet confirmation path
            // (Order/TransactionService::confirmTransaction) re-sends the full
            // address, so WeArePlanet customers are unaffected.
            $address->setCity('');
