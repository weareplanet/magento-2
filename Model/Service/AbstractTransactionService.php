...
use WeArePlanet\Sdk\Model\Gender;
...
abstract class AbstractTransactionService
{
     ...
     *
     * @param \WeArePlanet\Sdk\Model\PaymentMethodConfiguration[] $paymentMethods
     * @return void
     */
    ...
    protected function updateQuote(Quote $quote, Transaction $transaction)
    {
        $this->resource->getConnection()->update($this->resource->getTableName('quote'),
            [
                'weareplanet_space_id' => $transaction->getLinkedSpaceId(),
                'weareplanet_transaction_id' => $transaction->getId()
            ], [
    ...
    {
        return $this->cookieManager->getCookie('weareplanet_device_id');
    }
