...
use WeArePlanet\Payment\Model\Service\Quote\GiftCardAccountWrapper;
use WeArePlanet\Sdk\Model\LineItemAttributeCreate;
...
abstract class AbstractLineItemService
{
    ...
    protected function convertLineItems($entity)
    {
        ...
        $this->eventManager->dispatch('weareplanet_payment_convert_line_items',
            [
                'transport' => $transport,
                'entity' => $entity
            ]);
    ...
    private function convertItem($entityItem, $entity)
    {
        ...
        $this->eventManager->dispatch('weareplanet_payment_convert_product_line_item',
            [
                'transport' => $transport,
     ...
     *
     * @param \Magento\Quote\Model\Quote\Item|\Magento\Sales\Model\Order\Item|\Magento\Sales\Model\Order\Invoice\Item $entityItem
     * @return \WeArePlanet\Sdk\Model\LineItemAttributeCreate[]
     */
    ...
    protected function getCustomAttributes($productId, $storeId)
    {
        ...
        $productAttributeCodeConfig = $this->scopeConfig->getValue(
            'weareplanet_payment/line_items/product_attributes', ScopeInterface::SCOPE_STORE, $storeId);
        ...
        $shippingDiscountAmount, $shippingDescription)
    {
        if ($shippingAmount > 0) {
            ...
            if ($this->scopeConfig->getValue('weareplanet_payment/line_items/overwrite_shipping_description',
                ScopeInterface::SCOPE_STORE, $entity->getStoreId())) {
                $shippingItem->setName(
                    $this->helper->fixLength(
                        $this->scopeConfig->getValue(
                            'weareplanet_payment/line_items/custom_shipping_description',
            ...
            $this->eventManager->dispatch('weareplanet_payment_convert_shipping_line_item',
                [
                    'transport' => $transport,
                    'entity' => $entity
                ]);
