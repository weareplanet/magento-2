 ...
 */
namespace WeArePlanet\Payment\Model\Payment\Method;
...
/**
 * WeArePlanet payment method adapter.
 */
class Adapter extends \Magento\Payment\Model\Method\Adapter
{

    const CAPTURE_INVOICE_REGISTRY_KEY = 'weareplanet_payment_capture_invoice';
     ...
     *
     * @return \WeArePlanet\Payment\Model\PaymentMethodConfiguration
     */
    ...
    {
        $spaceViewId = $this->scopeConfig->getValue('weareplanet_payment/general/space_view_id');
    ...
    public function isAvailable(CartInterface $quote = null)
    {
        ...
        $spaceId = $this->scopeConfig->getValue(
            'weareplanet_payment/general/space_id',
        ...
        try {
            if (!$quote->getData('weareplanet_payment_payment_options_response')
             || ($quote->getData('weareplanet_payment_payment_tmp_currency') != $quote->getQuoteCurrencyCode())) {
                $payment_options_response = $this->transactionService->getPossiblePaymentMethods($quote);
                $quote->setData('weareplanet_payment_payment_options_response', $payment_options_response);
                $quote->setData('weareplanet_payment_payment_tmp_currency', $quote->getQuoteCurrencyCode());
                $quote->save();
            }

            $payment_options_response = $quote->getData('weareplanet_payment_payment_options_response');
